import { type User, type Task, type File, type Loop, type Iteration} from 'wasp/entities';
...
export const createLoop: CreateLoop<
  { name: string; description: string; accountabilityPartner: string },
  Loop
> = async ({ name, description, accountabilityPartner }, context) => {
  if (!context.user) {
